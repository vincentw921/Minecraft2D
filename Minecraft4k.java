...
public class Minecraft4k
    ...
    public static void main(String[] args) {
        ...
        for (int x = 0; x < CROSS_SIZE; x++) {
            for (int y = 0; y < CROSS_SIZE; y++) {
                if (((Math.abs(x - CROSS_SIZE / 2) + 2) * (Math.abs(y - CROSS_SIZE / 2) + 2) < Math
                        .sqrt((CROSS_SIZE * CROSS_SIZE / 2)))
                        && (Math.abs(x - CROSS_SIZE / 2) + 2) + (Math.abs(y - CROSS_SIZE / 2) + 2) < CROSS_SIZE
                                * 0.4375f
                        && (Math.abs(x - CROSS_SIZE / 2) + 2) + (Math.abs(y - CROSS_SIZE / 2) + 2) > CROSS_SIZE
                                * 0.296875f)
        ...
        hiddenCursor = frame.getToolkit().createCustomCursor(new BufferedImage(1, 1, BufferedImage.TYPE_INT_ARGB),
                new java.awt.Point(), null);
    ...
    static BufferedImage textureAtlasImage = new BufferedImage(TEXTURE_RES, 3 * 16 * TEXTURE_RES,
            BufferedImage.TYPE_INT_ARGB);
    static int[] textureAtlas = ((DataBufferInt) textureAtlasImage.getRaster().getDataBuffer()).getData();

    static byte[] hotbar = new byte[] { BLOCK_GRASS, BLOCK_DEFAULT_DIRT, BLOCK_STONE, BLOCK_BRICKS, BLOCK_WOOD,
            BLOCK_LEAVES };
        ...
        try {
            ...
            if (classic) {
                ...
                for (int x = 0; x < WORLD_SIZE; x++) {
                    for (int z = 0; z < WORLD_SIZE; z++) {
                        int terrainHeight = Math
                                .round(maxTerrainHeight + noise(x / halfWorldSize, z / halfWorldSize) * 10.0f);
                ...
                for (int x = 4; x < WORLD_SIZE - 4; x += 8) {
                    for (int z = 4; z < WORLD_SIZE - 4; z += 8) {
                        if (rand.nextInt(4) == 0) // spawn tree
                        {
                            ...
                            final int terrainHeight = Math.round(
                                    maxTerrainHeight + noise(treeX / halfWorldSize, treeZ / halfWorldSize) * 10.0f) - 1;
                            ...
                            // foliage
                            fillBox(BLOCK_LEAVES, treeX - 2, terrainHeight - treeHeight + 1, treeZ - 2, treeX + 3,
                                    terrainHeight - treeHeight + 3, treeZ + 3, false);

                            // crown
                            fillBox(BLOCK_LEAVES, treeX - 1, terrainHeight - treeHeight - 1, treeZ - 1, treeX + 2,
                                    terrainHeight - treeHeight + 1, treeZ + 2, false);
            ...
            for (int blockType = 1; blockType < 16; blockType++) {
                ...
                for (int y = 0; y < TEXTURE_RES * 3; y++) {
                    for (int x = 0; x < TEXTURE_RES; x++) {
                        ...
                        if (classic) {
                            if (blockType != BLOCK_STONE || rand.nextInt(3) == 0) // if the block type is stone, update
                                                                                  // the noise value less often to get a
                                                                                  // streched out look
                            ...
                            switch (blockType) {
                                case BLOCK_STONE:
                                    ...
                                    break;
                                case BLOCK_GRASS:
                                    if (y < (x * x * 3 + x * 81 >> 2 & 0x3) + (float) (TEXTURE_RES * 1.125f)) // grass +
                                                                                                              // grass
                                                                                                              // edge
                                        tint = 0x6AAA40; // green
                                    else if (y < (x * x * 3 + x * 81 >> 2 & 0x3) + (float) (TEXTURE_RES * 1.1875f)) // grass
                                                                                                                    // edge
                                                                                                                    // shadow
                                ...
                                case BLOCK_WOOD:
                                    ...
                                    if (!(y >= TEXTURE_RES && y < TEXTURE_RES * 2) && // second row = stripes
                                            x > 0 && x < TEXTURE_RES - 1 &&
                                            ((y > 0 && y < TEXTURE_RES - 1)
                                                    || (y > TEXTURE_RES * 2 && y < TEXTURE_RES * 3 - 1))) { // wood side
                                                                                                            // area
                                        tint = 0xBC9862; // light brown

                                        // the following code repurposes 2 gsd variables making it a bit hard to read
                                        // but in short it gets the absulte distance from the tile's center in x and y
                                        // direction
                                        // finds the max of it
                                        // uses that to make the gray scale detail darker if the current pixel is part
                                        // of an annual ring
                            ...
                            switch (blockType) {
                                case BLOCK_STONE:
                                    tint = 0x7F7F7F; // grey
                                    gsd_tempA = (int) ((0.75 + Math.round(Math.abs(noise(x * 0.5f, y * 2))) * 0.125f)
                                            * 255);
                                    break;
                                case BLOCK_GRASS:
                                ...
                                case BLOCK_WOOD:
                                    ...
                                    double distFromCenter = (Math.sqrt(dx * dx + dy * dy) * .25f
                                            + Math.max(dx, dy) * .75f);
                                    ...
                                    if (y < 16 || y > 32) { // top/bottom
                                        if (distFromCenter < TEXTURE_RES / 2) {
                                            ...
                                            gsd_tempA = (int) (noise(y, x * .25f) * 255
                                                    * (180 - Math.sin(x * Math.PI) * 50) / 100);
                                        } else {
                                            gsd_tempA = (int) (noise(x, y * .25f) * 255
                                                    * (180 - Math.sin(x * Math.PI) * 50) / 100);
                                        }
                                    } else { // side texture
                                        gsd_tempA = (int) (noise(x, y * .25f) * 255 * (180 - Math.sin(x * Math.PI) * 50)
                                                / 100);
                                    }
                                    break;
                                case BLOCK_BRICKS:
                                    ...
                                    float d = (float) (Math.sqrt(brickDX * brickDX + brickDY * brickDY) * .5f
                                            + Math.max(brickDX, brickDY) * .5f);
            ...
            while (true) {
                ...
                while (System.currentTimeMillis() - startTime > 10L) {
                    ...
                    startTime += 10L;
                    float inputX = (integer(input.contains(KeyEvent.VK_D)) - integer(input.contains(KeyEvent.VK_A)))
                            * 0.02F;
                    float inputZ = (integer(input.contains(KeyEvent.VK_W)) - integer(input.contains(KeyEvent.VK_S)))
                            * 0.02F;
                    velocityX *= 0.5F;
                    velocityY *= 0.99F;
                    velocityZ *= 0.5F;
                    ...
                    OUTER: for (int axisIndex = 0; axisIndex < 3; axisIndex++) {
                        ...
                        for (int colliderIndex = 0; colliderIndex < 12; colliderIndex++) {
                            ...
                            int colliderBlockY = (int) (newPlayerY + ((colliderIndex >> 2) - 1) * 0.8F + 0.65F)
                                    - WORLD_HEIGHT;
                            int colliderBlockZ = (int) (newPlayerZ + (colliderIndex >> 1 & 1) * 0.6F - 0.3F)
                                    - WORLD_SIZE;
                            ...
                            if (colliderBlockX < 0 || colliderBlockZ < 0
                                    || colliderBlockX >= WORLD_SIZE || colliderBlockY >= WORLD_HEIGHT
                                    || colliderBlockZ >= WORLD_SIZE
                ...
                for (int colliderIndex = 0; colliderIndex < 12; colliderIndex++) {
                    ...
                    if (magicX >= 0 && magicY >= 0 && magicZ >= 0 && magicX < WORLD_SIZE && magicY < WORLD_HEIGHT
                            && magicZ < WORLD_SIZE)
                ...
                if (hovered) {
                    ...
                    mouseDelta = new Point(mouseLocation.x - lastMouseLocation.x,
                            mouseLocation.y - lastMouseLocation.y);
    ...
    public void paint(java.awt.Graphics g) {
        ...
        for (int i = 0; i < hotbar.length; i++) {
            g2d.drawImage(textureAtlasImage,
                    ...
                    hotbarX + i * hotbarItemSize + hotbarItemSize - padding, hotbarY + hotbarItemSize - padding, // draw
                                                                                                                 // bounds
                                                                                                                 // max
...
class MinecraftEventListener extends java.awt.event.KeyAdapter implements java.awt.event.MouseListener,
        java.awt.event.MouseMotionListener, java.awt.event.FocusListener, java.awt.event.MouseWheelListener {
    @Override
    ...
    public void run() {
        while (true) {
            for (int screenIndex = 0; screenIndex < buffer.length; screenIndex++) {
                ...
                Vec3 rayDir = new Vec3(frustumRayX * cosYaw + temp * sinYaw, frustumRayY * cosPitch - sinPitch,
                ...
                for (int pass = 0; pass < (classic ? 1 : 2); pass++) {
                    AXIS: for (int axis = 0; axis < 3; axis++) {
                        ...
                        while (rayTravelDist < furthestHit) {
                            ...
                            if (blockHitX < 0 || blockHitY < -2 || blockHitZ < 0 || blockHitX >= WORLD_SIZE
                                    || blockHitY >= WORLD_HEIGHT || blockHitZ >= WORLD_SIZE)
                                break;
                            ...
                            if (blockHitID != BLOCK_AIR) {
                                ...
                                Vec3 textureColor;
                                if (pass == 0 &&
                                        (blockHitX == hoveredBlockPosX && blockHitY == hoveredBlockPosY
                                                && blockHitZ == hoveredBlockPosZ &&
                                                ((texFetchX == 0 || texFetchY % TEXTURE_RES == 0)
                                                        || (texFetchX == TEXTURE_RES - 1
                                                                || texFetchY % TEXTURE_RES == TEXTURE_RES - 1))))
                                    textureColor = new Vec3(1, 1, 1); // add white outline to hovered block
                                else
                                    textureColor = Vec3.fromRGB(textureAtlas[texFetchX + texFetchY * TEXTURE_RES
                                            + blockHitID * (TEXTURE_RES * TEXTURE_RES) * 3]);

                                int direction = 1;
                                if (delta > 0.0F)
                                    direction = -1;

                                if (rayTravelDist < hoverCheckDist && screenX == (SCR_RES_X * 2) / 4
                                        && screenY == (SCR_RES_Y * 2) / 4) {
                                    if (pass == 0) {
